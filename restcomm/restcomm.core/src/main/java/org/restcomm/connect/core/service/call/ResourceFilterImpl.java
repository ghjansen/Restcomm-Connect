    ...
    public boolean isOutboundVoiceCallUnderQuota(final Sid accountId) {
        if(active){
            try {
                ...
                builder.byAccountSid(String.valueOf(accountId));
                builder.byDirection("outbound-api");
                builder.byAge(outboundVoiceCallsPeriod);
                CallDetailRecordFilter cdrf = builder.build();
